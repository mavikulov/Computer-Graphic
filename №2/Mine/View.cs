...
namespace Vikulov_tomogramm_visualizer
{
    internal class View
    {
        ...
        public void generateTextureImage(int layer_number)
        {
            textureImage = new Bitmap(Bin.X, Bin.Y);
            for (int i = 0; i < Bin.X; ++i)
                for(int j = 0; j < Bin.Y; ++j)
                {
                    int pxl_number = i + j * Bin.X + layer_number * Bin.X * Bin.Y;
                    textureImage.SetPixel(i, j, TransferFunction(Bin.array[pxl_number]));
                }
        }
        ...
        public void DrawQuads(int layer_num, bool flag)
        {
            GL.Clear(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit);
            GL.Begin(BeginMode.Quads);
            for(int x_coord = 0; x_coord < Bin.X - 1; ++x_coord)
                for(int y_coord = 0; y_coord < Bin.Y - 1; ++y_coord)
                {
                    ...
                    else
                    {
                        ...
                        GL.Vertex2(x_coord, Bin.Y - y_coord - 1);
                        //2 вершина
                        value = Bin.array[x_coord + (y_coord + 1) * Bin.X
                            + layer_num * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        //GL.Vertex2(x_coord, y_coord + 1);
                        GL.Vertex2(x_coord, Bin.Y - y_coord);
                        //3 вершина
                        value = Bin.array[x_coord + 1 + (y_coord + 1) * Bin.X
                            + layer_num * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        //GL.Vertex2(x_coord + 1, y_coord + 1);
                        GL.Vertex2(x_coord + 1, Bin.Y - y_coord);
                        //4 вершина 
                        value = Bin.array[x_coord + 1 + y_coord * Bin.X
                            + layer_num * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        //GL.Vertex2(x_coord + 1, y_coord);
                        GL.Vertex2(x_coord + 1, Bin.Y - y_coord - 1);
                    }
